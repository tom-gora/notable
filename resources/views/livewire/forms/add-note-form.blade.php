<div class="max-w-8/12 relative flex h-full flex-col items-center justify-center pt-4 md:max-w-full">
    ...
    <x-mary-header :title="__('Click to upload an image or a scan of a note:')" class="forced-text-center forced-text-color border-b-text-subtle !mb-4 border-b"
        size="text-sm" />
    <x-mary-form class="flex h-full w-10/12 flex-col items-center justify-center" id="image-uploader"
        ...
        <x-mary-file :change-text="__('Select image')" :crop-after-change :crop-title-text="__('Crop the image')"
            accept="image/png, image/jpeg, image/jpg, image/webp" class="m-2 flex flex-col items-center justify-center"
            crop-save-text="__('OK')" error-class="form-error-custom" error-field="image_error" wire:model="note_image">
            <img class="w-40 rounded-lg" id="note-img-preview" src="{{ $note_img_url ?? $this->getDefaultImage() }}" />
        </x-mary-file>
        <x-mary-input :label="__('Note title...')" class="base-input w-[60vw] md:w-min" clearable error-class="form-error-custom"
            error-field="title_error" inline maxlength="50" minlength="3" wire:model="note_title" />
